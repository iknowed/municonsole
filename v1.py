...
from vehicle.models import Route
from vehicle.models import Run
...
def getNearestPath(self):
  ...
  sql = "select dp.path_id,ST_Distance( ST_GeomFromEWKT('SRID=100000;POINT("+ str(x) + " " + str(y) + ")'), pa.loc) from direction_path dp, direction d, path pa where d.tag=%s and dp.direction_id=d.id and pa.id=dp.path_id order by ST_Distance( ST_GeomFromEWKT('SRID=100000;POINT("+ str(x) + " " + str(y) + ")'), pa.loc),pa.loc asc"
  cur = self.db.cursor()
...
while(1):
  ...
  for vehicle in dom.getElementsByTagName("vehicle"):
    ...
    new.predictable = vehicle.getAttribute('predictable')
